...
**このドキュメントはv0.5.1のものです。他のバージョンのドキュメントを確認したい場合は、GitHubの過去のリリースもしくは[「以前のバージョンのドキュメント」ページ](versions)を確認してください。**
...
```sh
go install github.com/ikura-hamu/q-cli@v0.5.1
```
...
```sh
curl -OJL https://github.com/ikura-hamu/q-cli/releases/download/v0.5.1/q-cli_{OS}_{architecture}.(tar.gz|zip)
```
